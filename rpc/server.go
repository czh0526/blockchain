...
func (s *Server) RegisterName(name string, rcvr interface{}) error {
	...
	// 如果已经在 serviceRegistry 中注册过，合并 callbacks 和 subscriptions
	...
	// 没有注册过，在 serviceRegistry 中增加 Entry
	svc.name = name
	...
	log.Debug(fmt.Sprintf("[RPC]: Service Registered: %s => %s", svc.name, rcvr))
	return nil
}
...
func (s *Server) readRequest(codec ServerCodec) ([]*serverRequest, bool, Error) {
	// Reader ==> rpcRequest{id, service, method, *params, isPubSub}
	...
	for i, r := range reqs {
		...
		if r.isPubSub && strings.HasPrefix(r.method, unsubscribeMethodSuffix) {
			requests[i] = &serverRequest{id: r.id, isUnsubscribe: true}
			argTypes := []reflect.Type{reflect.TypeOf("")}
			// json.RawMessage ==> []reflect.Value
...
// 执行 serverRequest 函数，返回 Subscription ID
